...
def sync_settings():
    rows = []
    for k, v in st.session_state.costs.items(): rows.append({"種別": "COST", "項目名": k, "値1": v, "値2": "", "値3": ""})
    ...
    if not df_set.empty:
        ...
        for _, row in df_set[df_set["種別"] == "WORD"].iterrows():
            ...
            if w and w not in st.session_state.search_words:
                ...
                st.session_state.search_counts[w] = {"NJSS": int(pd.to_numeric(row["値1"], errors="coerce") or 0), "入札王": int(pd.to_numeric(row["値2"], errors="coerce") or 0), "登録日": str(row["値3"])}
...
def calc_proj():
    ...
    return pd.DataFrame(rows), ap, avg_bid
...
if current_page == "ダッシュボード":
    ...
    with kpi_col4: kpi("NJSS 5年利益", f"{int(n_p5/10000):,}", "万円", sub="累積期待利益", tag_type="neu", color="#8B5CF6", tooltip="【計算式】5年間の期待売上累計 － 5年間のNJSS利用コスト累計（詳細はROI分析画面を参照）")
    with kpi_col5: kpi("入札王 5年利益", f"{int(k_p5/10000):,}", "万円", sub="累積期待利益", tag_type="neu", color="#EC4899", tooltip="【計算式】5年間の期待売上累計 － 5年間の入札王利用コスト累計（詳細はROI分析画面を参照）")
    ...
    with st.container(border=True):
        sec("キーワード検索精度比較", note="ℹ️ 計算式: 「ワード検索数」画面で入力された各キーワードのヒット件数（実測値）")
        if st.session_state.search_words and st.session_state.search_counts:
            sw_df = pd.DataFrame([{"ワード": w, "NJSS": st.session_state.search_counts.get(w,{}).get("NJSS",0), "入札王": st.session_state.search_counts.get(w,{}).get("入札王",0)} for w in st.session_state.search_words])
            fig3 = px.bar(sw_df, x="ワード", y=["NJSS","入札王"], barmode="group", color_discrete_map={"NJSS": C1, "入札王": C2})
            fig3.update_traces(marker_line_width=0); fig3.update_layout(**PLY, height=350, legend_title_text=""); fig3.update_xaxes(tickangle=-45); fig3.update_yaxes(title="ヒット件数", gridcolor="rgba(0,0,0,0.05)", zeroline=False)
            st.plotly_chart(fig3, use_container_width=True)
        else: st.caption("「ワード検索数」画面からデータを追加してください。")

    r3l, r3r = st.columns(2)
    with r3l:
        with st.container(border=True):
            sec("競合出現シェア", note="ℹ️ 計算式: 登録案件の「落札企業」「競合1〜3」に登場した企業名の合計出現回数（上位10社）")
            comp = pd.concat([vd["落札企業"], vd["競合1"], vd["競合2"], vd["競合3"]])
            comp_df = comp[comp.notna() & (comp != "")].value_counts().head(10).reset_index(); comp_df.columns = ["企業名", "回数"]
            ...
            fig2.update_layout(**PLY, showlegend=False, height=h); fig2.update_yaxes(autorange="reversed"); fig2.update_xaxes(title="", gridcolor="rgba(0,0,0,0.05)", zeroline=False)
    ...
    with r3r:
        with st.container(border=True):
            ...
            with v2:
                ...
                st.markdown(f'<div style="font-size:11px;font-weight:700;color:#64748B;margin-bottom:8px;">2. 検索精度</div><div style="font-size:1.5rem;font-weight:800;color:{color2};">{label2}</div><div style="font-size:11px;color:#64748B;margin-top:4px;">優位ワード数比較</div>', unsafe_allow_html=True)
            with v3:
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            if nj_sc > ki_sc: st.markdown(f'<div class="verdict"><div class="verdict-title">最終推奨：NJSS ({nj_sc}/3項目)</div><div class="verdict-body">各検証データを総合した結果、NJSSの導入を推奨します。過去案件の網羅性と機会損失防止の観点で優位性が確認されました。</div></div>', unsafe_allow_html=True)
            elif ki_sc > nj_sc: st.markdown(f'<div class="verdict"><div class="verdict-title">最終推奨：入札王 ({ki_sc}/3項目)</div><div class="verdict-body">各検証データを総合した結果、入札王の導入を推奨します。コストパフォーマンスと早期損益分岐点の優位性により、ROI最大化が期待できます。</div></div>', unsafe_allow_html=True)
            else: st.info("両者拮抗 (引き分け)。UIの使いやすさや営業サポート体制など定性要素で最終判断してください。")

    with st.container(border=True):
        sec("累積期待利益の予測推移（5カ年）", note="ℹ️ 計算式: (年間期待売上 × 経過年数) － 累積利用コスト")
        fig4 = px.line(p_df, x="年", y=["NJSS利益","入札王利益"], color_discrete_map={"NJSS利益": C1, "入札王利益": C2})
        fig4.update_traces(line_width=3); fig4.update_layout(**PLY, height=300); fig4.update_yaxes(title="累積利益 (円)", gridcolor="rgba(0,0,0,0.05)", zeroline=False); fig4.update_xaxes(title="経過年数")
...
# ─────────────────────────────────────────────────────────────────
    ...
    with st.container(border=True):
        ...
        if ca2.button("追加", use_container_width=True):
            if nw and nw not in st.session_state.search_words:
                st.session_state.search_words.append(nw); st.session_state.search_counts[nw] = {"NJSS": 0, "入札王": 0, "登録日": today_str}
    ...
    with st.container(border=True):
        ...
        if st.session_state.search_words:
            df_sw = pd.DataFrame([{"検索ワード": w, "登録日": st.session_state.search_counts.get(w,{}).get("登録日", today_str), "NJSS (件)":  st.session_state.search_counts.get(w,{}).get("NJSS",0), "入札王 (件)": st.session_state.search_counts.get(w,{}).get("入札王",0)} for w in st.session_state.search_words])
            ...
            if st.button("件数を保存してダッシュボードへ反映", use_container_width=True):
                ...
                st.session_state.search_counts = {row["検索ワード"]: {"NJSS": int(row.get("NJSS (件)",0) or 0), "入札王": int(row.get("入札王 (件)",0) or 0), "登録日": str(row.get("登録日", today_str))} for _, row in edited.iterrows() if pd.notna(row["検索ワード"])}
...
# ─────────────────────────────────────────────────────────────────
    ...
    with st.container(border=True):
        ...
        wr = s1.number_input("平均受注率 (%)", value=st.session_state.costs["win_rate"], help="これまでのプロポーザル等の勝率（目標値でも可）を入力します。")
        mg = s2.number_input("平均粗利率 (%)", value=st.session_state.costs["margin"], help="案件の売上に対する利益率を入力します。")
        ab = s3.number_input("年間想定応札数 (件)", value=st.session_state.costs["annual_bids"], help="年間で入札に参加する想定件数を入力します。")
    ...
    with st.container(border=True):
        ...
        fb.update_layout(**PLY, height=300); fb.update_yaxes(title="金額 (円)", gridcolor="rgba(0,0,0,0.05)", zeroline=False); fb.update_xaxes(title="経過年数")
        st.plotly_chart(fb, use_container_width=True)

    with st.container(border=True):
        sec("各年の累積利益比較")
        fp = px.bar(p_df, x="年", y=["NJSS利益","入札王利益"], barmode="group", color_discrete_map={"NJSS利益": C1, "入札王利益": C2})
        fp.update_traces(marker_line_width=0); fp.update_layout(**PLY, height=280); fp.update_yaxes(title="累積利益 (円)", gridcolor="rgba(0,0,0,0.05)", zeroline=False)
                    ...
                    for _, row in im.iterrows():
                        ...
                        if tag == "SETTING_COST":
                            ...
                            if w:
                                ...
                                st.session_state.search_counts[w] = {"NJSS": int(pd.to_numeric(row.get("案件概要",0), errors="coerce") or 0), "入札王": int(pd.to_numeric(row.get("落札企業",0), errors="coerce") or 0), "登録日": today_str}
                        else:
            ...
            if st.button("全データを初期化する", use_container_width=True):
                if ok:
                    try:
                        ...
                        st.session_state.update({"search_words": [], "search_counts": {}, "costs": {"n_init":0,"n_month":0,"n_opt":0,"k_init":0,"k_month":0,"k_opt":0,"margin":20,"win_rate":20,"annual_bids":50}})
                        st.success("初期化完了。")
                    except Exception as e: st.error(f"エラー: {e}")
