...
import plotly.graph_objects as go  # 💡追加：レーダーチャート描画用
from streamlit_gsheets import GSheetsConnection
import io
import csv
import traceback

# --- 1. UI & CSS (アイコン排除 & シャープなSaaS風UI) ---
...
st.markdown("""
    ...
    /* ページヘッダー */
    .slds-page-header { background-color: #FFFFFF !important; padding: 1.5rem 2rem; border-bottom: 1px solid #E2E8F0; margin: -2rem -2rem 2rem -2rem; }
...
def load_data():
    try:
        ...
        if "自治体名" not in df.columns: return pd.DataFrame([{col: "" if col != "ID" else i+1 for col in CORRECT_COLUMNS} for i in range(50)])
...
with st.sidebar:
    ...
    page = st.radio(
        "メニュー",
        menu_options,
        format_func=lambda x: x,  # 💡 アイコンを完全に排除し、文字だけを表示
...
if page == "ダッシュボード":
    ...
    if valid_df.empty:
        ...
        if st.button("総合判定を実行する", use_container_width=True):
            ...
            # レーダーチャート描画
            categories = ['網羅率', '検索精度', 'コストパフォーマンス', 'ROI(投資対効果)', '網羅率'] # 閉じるために最後を繰り返す
            fig_radar = go.Figure()
            ...
            with rc2:
                st.markdown("### 🏆 判定結果")
                if nj_total > ki_total:
                    st.success("分析の結果、**【 NJSS 】** の導入が推奨されます。")
                elif ki_total > nj_total:
                    st.success("分析の結果、**【 入札王 】** の導入が推奨されます。")
                else:
                    st.info("分析の結果、両ツールは **【 互角 】** です。")
