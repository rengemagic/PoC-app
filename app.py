...
if not st.session_state.logged_in:
    # ログイン画面専用のスタイル（背景を真っ白にしてロゴを同化させる）
    st.markdown("""
        <style>
...
def draw_kpi_card(title, value):
    # 💡 CSSクラス「kpi-card」を適用して立体化
    st.markdown(f"""
        <div class="kpi-card">
            <p style="color: #64748B; font-size: 13px; font-weight: 700; margin: 0 0 8px 0; letter-spacing: 0.5px;">{title}</p>
            <p style="color: #0176D3; font-size: 32px; font-weight: 800; margin: 0; line-height: 1.2;">{value}</p>
        </div>
    """, unsafe_allow_html=True)
...
if page == "ダッシュボード":
    ...
    if valid_df.empty:
        ...
        with col_l:
            ...
            fig_hits.update_layout(template="plotly_white", xaxis_title=None, yaxis_title=None, margin=dict(l=0, r=0, t=40, b=0), paper_bgcolor="rgba(0,0,0,0)")
        ...
        with col_r:
            ...
            fig_p.update_layout(template="plotly_white", xaxis_title=None, yaxis_title=None, margin=dict(l=0, r=0, t=40, b=0), paper_bgcolor="rgba(0,0,0,0)")
        ...
        if st.session_state.search_words and st.session_state.search_counts:
            ...
            if not df_sw.empty:
                ...
                fig_sw.update_layout(template="plotly_white", xaxis_title=None, yaxis_title=None, margin=dict(l=0, r=0, t=20, b=0), paper_bgcolor="rgba(0,0,0,0)")
        ...
        fig_proj.update_layout(template="plotly_white", xaxis_title=None, yaxis_title=None, margin=dict(l=0, r=0, t=20, b=0), paper_bgcolor="rgba(0,0,0,0)")
        fig_proj.data[0].name="NJSS利益"; fig_proj.data[1].name="入札王利益"
    ...
    with st.form("entry_form", clear_on_submit=True):
        ...
        # 💡 URL入力枠を5つ追加
        st.markdown("#### 関連URL (最大5件)")
        u_cols = st.columns(2)
        url1 = u_cols[0].text_input("URL 1", placeholder="https://...", label_visibility="collapsed")
        url2 = u_cols[1].text_input("URL 2", placeholder="https://...", label_visibility="collapsed")
        url3 = u_cols[0].text_input("URL 3", placeholder="https://...", label_visibility="collapsed")
        url4 = u_cols[1].text_input("URL 4", placeholder="https://...", label_visibility="collapsed")
        url5 = u_cols[0].text_input("URL 5", placeholder="https://...", label_visibility="collapsed")
        ...
        if st.form_submit_button("この案件を保存する", use_container_width=True):
            if mun:
                ...
                new_rec = pd.DataFrame([{
                    "ID": len(valid_df)+1, "自治体名": mun, "案件概要": smm, "仕様書": spc, "予算(千円)": 0, 
                    "落札金額(千円)": wbid, "落札企業": wnr, "応札1": b1, "応札2": b2, "応札3": "", 
                    "NJSS掲載": njl, "入札王掲載": kil,
    ...
    fig_prof.data[0].name="NJSS利益"; fig_prof.data[1].name="入札王利益"
    ...
    **STEP 5: 最終判断と稟議**
    ...
    sample_data = [
        ...
        {"ID": 1, "自治体名": "東京都", "案件概要": "ダッシュボード構築", "仕様書": True, "予算(千円)": 0, "落札金額(千円)": 15000, "落札企業": "株式会社ジール", "応札1": "A社", "応札2": "B社", "応札3": "", "NJSS掲載": True, "入札王掲載": False, "URL1": "", "URL2": "", "URL3": "", "URL4": "", "URL5": ""},
        {"ID": 2, "自治体名": "大阪府", "案件概要": "BIツールライセンス更新", "仕様書": True, "予算(千円)": 0, "落札金額(千円)": 8000, "落札企業": "C社", "応札1": "株式会社ジール", "応札2": "", "応札3": "", "NJSS掲載": True, "入札王掲載": True, "URL1": "", "URL2": "", "URL3": "", "URL4": "", "URL5": ""}
    ]
